...
int main(void){
    ...
    int index = 0; // 최소값의 인덱스를 저장할 변수, 초기값은 첫 번째 요소의 인덱스

    // 배열에서 최소값의 인덱스 찾기
    for(int i = 1; i < SIZE; i++){ // i는 1부터 SIZE-1(9)까지 반복
        if(prices[index] > prices[i]){ // 현재 최소값보다 작은 값이 나오면
            printf("%d\t%d\n", i, index); // 현재 인덱스와 최소값의 인덱스 출력
            printf("%d\t%d\n\n", prices[i], prices[index]); // 현재 값과 최소값 출력
            index = i; // 최소값의 인덱스를 갱신
        }
    }
    printf("\nmin:%d", prices[index]); // 최소값 출력
