...
double current_millis() {
    ...
    QueryPerformanceFrequency(&frequency); // 주파수 측정
    QueryPerformanceCounter(&current_time); // 현재 시간 측정
    return (double)(current_time.QuadPart * 1000) / frequency.QuadPart; // 밀리초 단위로 변환
}
...
int main(int argc, char *argv[]){
    ...
    //배열 a1coef, a1exp 선언
    int* a1coef = (int*)malloc(sizeof(int) * n1);
    int* a1exp = (int*)malloc(sizeof(int) * n1);
    //배열 a2coef, a2exp 선언
    int* a2coef = (int*)malloc(sizeof(int) * n2);
    int* a2exp = (int*)malloc(sizeof(int) * n2);
    ...
    int sum = 0; 
    while(!IsZero(a1) && !IsZero(a2)){ // 두 다항식이 비어있지 않을 때까지 반복
        switch (COMPARE(Lead_Exp(a1), Lead_Exp(a2))){ // a1과 a2의 최고차항 지수 비교
            case -1: // a1 < a2
                d = Attach(d, Coef(a2, Lead_Exp(a2)), Lead_Exp(a2)); // a2의 항을 d에 추가
                a2 = Remove(a2, Lead_Exp(a2)); // a2에서 해당 항 제거
                break;
            case 0: // a1 = a2
                sum = Coef(a1, Lead_Exp(a1)) + Coef(a2, Lead_Exp(a2)); // 동일 차수 계수 합
                if(sum != 0){ // 합이 0이 아니면
                    d = Attach(d, sum, Lead_Exp(a1)); // d에 합 추가
                }
                else{ // 합이 0이면
                    d = Attach(d, 0, Lead_Exp(a1)); // d에 0 추가
                }
                if(sum){ // 합이 0이 아니면
                    Attach(d, sum, Lead_Exp(a1)); // d에 합 추가
    ...
    //구조체 poly 선언
    poly = (polynomial1 *)malloc(sizeof(polynomial1)* (n1 + n2 + 1));
    ...
    *avail = n1 + n2; // 새로운 항 저장 위치 = 결과 다항식 시작 위치
    ...
    //구조체 s1, s2 선언
    term *ls1 = (term *)malloc(sizeof(term) * n1);
    term *ls2 = (term *)malloc(sizeof(term) * n2);
